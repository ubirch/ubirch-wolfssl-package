...
function build_software() {
  docker run --user `id -u`:`id -g` -e "HOME=/build" --workdir=/build/ubirch-wolfssl-package --volume=${PWD}/..:/build --env=${TOOLCHAIN} --env=${WOLFSSL_ROOT} --entrypoint=/bin/bash ubirch/arm-build:${ARM_CONTAINER_VERSION} ./build.sh
  if [ $? -ne 0 ]; then
